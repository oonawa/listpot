		...
		<>
			<div className="w-full flex items-center gap-2 pt-2">
				{allSubLists.length > 0 && (
					...
					>
						<SelectTrigger className="rounded-full font-bold border-background-light-2 overflow-hidden *:data-[slot=select-value]:inline-block *:data-[slot=select-value]:truncate">
							<SelectValue />
						</SelectTrigger>
						<SelectContent
							position="popper"
				...
				<WatchTogglePill
					isActive={includeWatched}
					onToggle={() => {
						setIncludeWatched(!includeWatched);
						resetResult();
					}}
					ariaLabel={includeWatched ? "観た作品を含めない" : "観た作品を含める"}
				>
					<span
						className={`font-bold text-sm text-foreground-dark-1 flex ${includeWatched ? "" : "text-foreground-dark-2 hover:text-foreground"}`}
					>
						観た作品を含める
					</span>
				</WatchTogglePill>
