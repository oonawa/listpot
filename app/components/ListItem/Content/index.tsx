...
export default function Content({
	...
	// TMDB がポスターも背景も持たない作品がある。どちらも無いときは details 無しと
	// 同じ空状態（ポスターなし＋再検索）を見せる。
	const hasEyeCatchImage = Boolean(
		movie.details &&
			(movie.details.posterImage || movie.details.backgroundImage),
	);

	const eyeCatchInner =
		movie.details && hasEyeCatchImage ? (
			<MovieDetail
	...
	return (
		<>
			...
			<h2 className="text-lg sm:text-xl font-bold inline-fit mx-auto">
				<span className="inline-flex pb-2 pt-4 sm:pb-4 sm:pt-6">
					{movie.title}
				</span>
			</h2>
